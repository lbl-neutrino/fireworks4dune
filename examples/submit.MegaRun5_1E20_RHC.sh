...
workflows/submit.MegaRun5_1E20_RHC.py

lpad admin tuneup --full

sbatch -o "$logdir"/slurm-%j.txt --array=1-10 -N 4 -t 180 slurm/fw_cpu.slurm.sh mega_cpu_hours rapidfire
...
scripts/fwsub.py --runner SimFor2x2_v4_Plots --base-env "$name.plots" --name "$name.plots.test" --size 10000 --start 0
